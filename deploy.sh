...
ssh -i "$KEY" "$HOST" "mkdir -p ~/$REMOTE_DIR && tar -C ~/$REMOTE_DIR -xzf ~/site-travels.tgz && rm ~/site-travels.tgz && chmod 600 ~/$REMOTE_DIR/.user.ini 2>/dev/null; ls ~/$REMOTE_DIR | head -20"
