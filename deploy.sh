#!/usr/bin/env bash
# Bygger Traveldays og deployer til Domeneshop-webhotellet for mollan-olafsen.fr (~/travels, adresse travels.mollan-olafsen.fr) over SSH.
# Bevarer alltid ~/travels_private (config + krypterte bilder) – den ligger utenfor webroten og røres aldri.
set -euo pipefail
cd "$(dirname "$0")"

HOST="${TD_HOST:-mollan-olafsenfr@login.domeneshop.no}"
